defmodule Zookeeper.Client do
  ...
  @spec init({binary | [{list, integer}], any, any}) ::
          {:ok, %{stop_on_disconnect: any, watchers: %{}, zk: pid}}
  def init({hosts, timeout, stop_on_disconnect}) do

    case hosts
        |> parse_hosts
        |> :erlzk_conn.start_link(timeout, monitor: self()) do
          {:ok, pid} -> {:ok, %{zk: pid, watchers: %{}, stop_on_disconnect: stop_on_disconnect}}
          {:error, :no_available_server} -> {:error, :no_available_server}
    end
  end
